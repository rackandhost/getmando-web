...
- The documented `dashboard.yaml` volume mount moves from `:ro` to `:rw` so the configurator's "Save" action can write to it. Existing deployments are unaffected on upgrade: without `CONFIG_WRITE_TOKEN`, or with the volume left `:ro`, the dashboard serves exactly as before and only the new save action fails, with copy and download still available.
